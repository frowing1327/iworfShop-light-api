...
public class IdentityService : IIdentityService
{
    ...
    private readonly JwtOptionsService _jwtOptionsService;
    ...
    public async Task<string> GenerateJwtToken(User user)
    {
        ...
        try
        {
            var key = new SymmetricSecurityKey(
                Encoding.UTF8.GetBytes(await _jwtOptionsService.GetJwtValueAsync("key")));
            var creds = new SigningCredentials(key, SecurityAlgorithms.HmacSha256);

            var tokenAttributes = new JwtSecurityToken(
                issuer: await _jwtOptionsService.GetJwtValueAsync("issuer"),
                audience: await _jwtOptionsService.GetJwtValueAsync("audience"),
                claims: claims,
